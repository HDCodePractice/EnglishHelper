...
from dict.user_chapter_dict import (clear_user_config, gen_user_chapter_dict,
                                    set_user_config, stored_data,
                                    update_data_to_file)
...
def update_topic_list(chapter_id, user_id, stored_data):
    ...
    for topic in topic_list:
        ...
        topic_menu_keyboard.append([
            InlineKeyboardButton(
                text=f"{count} {topic_status}", callback_data=f"topic-select:{chapter_id}:{topic}:{topic_status}:{user_id}")
        ])
        count += 1
    topic_menu_keyboard.append([
        InlineKeyboardButton(
...
def custom_chapter_command(update: Update, context: CallbackContext) -> None:
    ...
    print(
        f"=============================\n {stored_data} \n=============================")
    chapter_preview_msg, menu_keyboard = update_chapter_list(
        str(user_id), stored_data)
...
def handle_menu_callback(update: Update, context: CallbackContext) -> None:
    ...
    if data[0] == "custom-select":
        if data[-2] == status["0"]:
            set_user_config(data[-1], data[1],
                            [list(), list(chapter_dict[data[1]].keys())])
        if data[-2] == status["1"] or data[-2] == status["2"]:
            set_user_config(data[-1], data[1],
                            [list(chapter_dict[data[1]].keys()), list()])
        chapter_preview_msg, menu_keyboard = update_chapter_list(
            data[-1], stored_data)
